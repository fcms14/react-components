    ...
    <>
      {items.map(({ rightTitle, rightText, leftTitle, leftText, leftIcon, rightIcon, onClick, observer }, key: number) => (
        <Row.Root key={key} rowStyle={{ borderBottom: `1px solid ${theme.colors.main.stroke}` }} observer={observer}>
          ...
          <Row.Section>
            <Row.Text size="small" textWrap="wrap">{leftText}</Row.Text>
            <Row.Subtitle size="small">{leftTitle}</Row.Subtitle>
          </Row.Section>
          <Row.Section sectionStyle={{ alignItems: "flex-end" }}>
            <Row.Text size="small" textAlign="right">{rightText}</Row.Text>
            <Row.Subtitle  size="small" textWrap="nowrap">{rightTitle}</Row.Subtitle>
          </Row.Section>
